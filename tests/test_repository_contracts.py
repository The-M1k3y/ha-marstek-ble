"""Repository, packaging and translation contracts for the original integration."""
...
def test_declared_platforms_have_modules_and_setup_entrypoints() -> None:
    ...
    for platform in platforms:
        ...
        functions = {
            node.name for node in module_tree.body if isinstance(node, ast.AsyncFunctionDef)
        }
...
def test_config_flow_abort_reasons_and_steps_have_strings() -> None:
    ...
    abort_reasons = set(re.findall(r'async_abort\(reason="([^"]+)"\)', source))
...
def test_original_modules_do_not_import_branch_only_product_model() -> None:
    forbidden = {"schema", "entity", "products"}
    ...
    for filename in ORIGINAL_MODULES:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.ImportFrom) and node.level:
                ...
                assert module_root not in forbidden, f"{path} imports .{node.module}"
            elif isinstance(node, ast.Import):
                for alias in node.names:
                    parts = alias.name.split(".")
                    if "custom_components" in parts and "marstek_ble" in parts:
                        assert forbidden.isdisjoint(parts), f"{path} imports {alias.name}"
...
def test_original_python_sources_compile_without_importing_dependencies() -> None:
    ...
    paths.append(ROOT / "standalone_test" / "marstek_basic_info.py")

    for path in paths:
        compile(path.read_text(encoding="utf-8"), str(path), "exec")


def test_coverage_configuration_excludes_only_branch_scaffolding() -> None:
    coverage = (ROOT / ".coveragerc").read_text(encoding="utf-8")

    assert "custom_components/marstek_ble/schema.py" in coverage
    assert "custom_components/marstek_ble/entity.py" in coverage
    assert "custom_components/marstek_ble/products/*" in coverage
    for filename in ORIGINAL_MODULES:
